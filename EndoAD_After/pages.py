...
class Waitforall(WaitPage):
    ...
    def after_all_players_arrive(self):
        ...
        for p in self.group.get_players():
            app4_list = p.participant.vars.get('app4_payoffs',[])
            app4_list.append(p.round_payoff)
            p.participant.vars['app4_payoffs'] = app4_list

        for p in self.group.get_players():
            if self.round_number == Constants.num_rounds:
                payoff_list = p.participant.vars.get('payoffs', [])
                app4_list = p.participant.vars.get('app4_payoffs', [])
                payoff_list.append(app4_list)
...
page_sequence = [init_calc, Instructions_first, Question, Observation, WaitforP2, Sender_report, WaitforP1, Receiver_guess, Waitforall, Results]
